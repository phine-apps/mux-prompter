...
- **Robust placeholder resolution**: Implemented non-recursive sentinel token replacement preventing secondary placeholder injection, resolved multiple `{{file:lines=...}}` ranges in a single template, and preserved command spaces and multi-space prompt layouts in `{{last_command}}`.
